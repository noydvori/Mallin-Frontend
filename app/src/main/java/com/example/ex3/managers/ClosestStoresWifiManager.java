...
public class ClosestStoresWifiManager {
    ...
    public ClosestStoresWifiManager(Context context) {
    ...
    private void sendScanResultsToServer(List<ScanResult> scanResults) {
        ...
        String token = UserPreferencesUtils.getToken(this.context);
        LocationAPI.getInstance().getClosestStores(token, scanResults).thenAccept(closestStores -> {
            UserPreferencesUtils.setClosestStores(this.context, closestStores);
